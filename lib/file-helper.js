    ...
    return function (stylesheet) {
        ...
        const css = stylesheet.contents.toString().replace(/url\(['"]?([^'"\\)]+)['"]?\)/g, (match, assetPath) => {
            ...
            const pathPrefix = (typeof opts.pathPrefix === 'undefined') ? pathPrefixDefault : opts.pathPrefix;
 			...
 			if (html.history && html.history[0] && isExternal(html.history[0])) { // If html remote downloaded
                 const index = html.stylesheets.indexOf(stylesheet.history[0]);
                 const stylesheets = html.originStylesheets;
                 const originUrlCss = stylesheets[index];
                 const remoteUrlAsset = new URL(assetPath, originUrlCss);
                 return normalizePath(match.replace(assetPath, remoteUrlAsset.href));
             }			
